...
import type { Town, CharacterStats, AlchemyRecipe } from '../types/game';
import { ALCHEMY_RECIPES, ITEM_DATABASE } from '../types/game';
...
export const TownScreen: React.FC<TownScreenProps> = ({ town, player, onLeave, onCraftAlchemy }) => {
                        ...
                        {activeFacility === 'alchemy' ? (
                            ...
                            <div className="flex-1 glass-panel rounded-3xl border border-white/10 p-8 flex flex-col items-center justify-center text-center">
                                <div className="text-6xl mb-6 opacity-50 animate-bounce">
                                    {activeFacility === 'market' ? '💰' : activeFacility === 'blacksmith' ? '🔥' : '🧪'}
                                </div>
