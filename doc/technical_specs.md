...
### 細粒度差異化同步 (Granular Selective Sync)
進階版的差異化同步。將重型資料（夥伴、裝備、道具、建築等）拆分為各自獨立的比對快照（`lastSyncedHashRef`）。
*   **效益**：若玩家單純撿到一個道具，系統**只會傳送變化的道具陣列**，而不會連帶傳輸數 KB 的夥伴與裝備資料，單次傳輸量可再縮減 60% ~ 80%。
...
## 更新紀錄 (Update History)
*   **2026-03-06**：實作細粒度差異化同步 (Granular Selective Sync)，根據資料類別分開傳輸以縮減 Payload。
